...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        ## ours ###
        self.mlp_1 = _MLP(in_features=hidden_dim, hidden_features=hidden_dim, act_layer=mlp_act_layer, drop=0, channels_first=channel_first)
        self.mlp_2 = _MLP(in_features=hidden_dim, hidden_features=hidden_dim, act_layer=mlp_act_layer, drop=0, channels_first=channel_first)
        mu = torch.Tensor(1, hidden_dim, 64)
        mu.normal_(0, math.sqrt(2. / 64))    # Init with Kaiming Norm.
...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # import numpy as np
        # import time
        # np.save('cor_vis/rldr_em_{}.npy'.format(time.time()), feature.cpu().numpy())
        # print(feature.shape)
        # exit()
        x = self.classifier(x)
        return feature, x, feature_
        # return feature, x
