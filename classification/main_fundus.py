...
from models.losses import DahLoss
...
def parse_option():
    ...
    parser.add_argument('--source_domain', type=str, nargs="+")
...
def main(config, args):
    ...
    # # GDRNet
    # criterion = DahLoss(beta= 0.5, max_iteration = config.TRAIN.EPOCHS, training_domains = config.SOURCE_DOMAIN, temperature = 0.1, scaling_factor = 4)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, model_time_warmup=50):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            features, outputs = model(samples)
        loss = criterion(outputs, targets)
        # loss, _ = criterion([outputs], [features, features], torch.argmax(targets, dim=1), torch.zeros((targets.shape[0])).long().cuda())
...
def validate(config, data_loader, model):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            ...
            feature, output, feat = model(images)
            feat = feat.sum(dim=-1)
            # print(feat.shape)
            feat = feat.cpu().numpy()
            if idx == 0:
            ...
            # np.save('heat_vis/feat/aptos_stage4.npy', feat)
            # exit()
        # measure accuracy and record loss
        loss = criterion(output, target)
        # loss, _ = criterion([output], [feature, feature], target, torch.zeros((target.shape[0])).long().cuda())
        
        ### ours ###
        softmax = torch.nn.Softmax(dim=1)
        _, pred = torch.max(output, 1)
        output_sf = softmax(output)
        label = torch.nn.functional.one_hot(target, num_classes=config.MODEL.NUM_CLASSES)
    ...
    ### heatmap
    # np.save('heat_vis/feat/rldr_stage2.npy', feats)
    ...
    # target1 = [item for sublist in target_list for item in sublist]
    # pred1 = [item for sublist in pred_list for item in sublist]
    # target1 = np.array(target1)
    # pred1 = np.array(pred1)
    # cm = confusion_matrix(target1, pred1)
    ...
    # plt.xlabel("Pred")
    # plt.ylabel("Label")
    # plt.title("ESDG-FGADR")
    # # plt.show()
    # plt.savefig('conf_vis/cm_ESDG-FGADR.png')
    # exit()
    
    ### ours ###
    label = [item for sublist in label_list for item in sublist]
    target_ = [item for sublist in target_list for item in sublist]
    pred = [item for sublist in pred_list for item in sublist]
    output_ = [item for sublist in output_list for item in sublist]
    f1 = f1_score(target_, pred, average='macro')
    auc_ovo = roc_auc_score(label, output_, average='macro', multi_class='ovo')
    BA = balanced_accuracy_score(target_, pred)
    n_c = np.zeros(config.MODEL.NUM_CLASSES)
    c_c = np.zeros(config.MODEL.NUM_CLASSES)
    for i in range(len(target_)):
        n_c[target_[i]] += 1
        if target_[i] == pred[i]:
            c_c[target_[i]] += 1
    acc_c = c_c / n_c
    logger.info(f' * Acc@1 {acc1_meter.avg:.3f} Acc@5 {acc5_meter.avg:.3f} AUC {auc_ovo:.3f} F1 {f1:.3f} BA {BA:.3f}')
    logger.info(f' * Acc@c1 {acc_c[0]:.3f} Acc@c2 {acc_c[1]:.3f} Acc@c3 {acc_c[2]:.3f} Acc@c4 {acc_c[3]:.3f} Acc@c5 {acc_c[4]:.3f}')
    ### ours ###
